...
function shapeAll() {
    if (/(iPhone|iPod|iPad).*AppleWebKit/i.test(window.navigator.userAgent) != true) {
        return;
    }

    console.log("document ready");
    $(".salahName").each(function (index) {
        console.log(this);
        $(this).html(convert($(this).html()));
    });

    $("span").each(function (index) {//.menuItem
        console.log(this);
        $(this).html(convert($(this).html()));
    });

    var elmnts = ["City", "WeekDay", "Month", "btnAbout", "btnCity"];
    for (var n = 0; n < elmnts.length; n++) {
        console.log(elmnts[n]);
        $("#" + elmnts[n]).html(convert($("#" + elmnts[n]).html()));
    }
    $(".modal-body").html(convert($(".modal-body").html()));
}
